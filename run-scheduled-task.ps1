...
try {
    ...
    Write-Host "`nRunning '$TaskName' on '$RemoteComputer'" -ForegroundColor Green
    Start-ScheduledTask -InputObject $task -CimSession $session -ErrorAction Stop

    Write-Host "`nTask: '$TaskName' started successfully on '$RemoteComputer'." -ForegroundColor Green
